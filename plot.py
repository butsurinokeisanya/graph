...
plt.yticks(np.arange(YTMIN, YTMAX+1, step=YINC))
...
plt.plot(x, y1,         "+", color="r",  markersize=PSIZE, label=header[1])
plt.plot(x, y2,         "-", color="c",  linewidth=DLW,    label=header[2], zorder=-1)
plt.plot(x, y3+OFFSETD, "-", color="b",  linewidth=DLW,    label=header[3])
plt.plot(x, y4,         "-", color="k",  linewidth=DLW,    label=header[4])
plt.errorbar(x2,[OFFSET1]*len(x2),fmt="none", color="g", yerr=LEN_BAR, label="Expected Reflection")


# タイトル
plt.title("Fluorapatite, Ca$_5$(PO$_4$)$_3$", y=1.05)
ax.legend(frameon=False) #フレームなし
plt.ylabel("Intensity [arb.units]", labelpad=SYLBL)
